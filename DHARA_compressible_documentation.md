...
Runs the 2D compressible Taylor–Green vortex at 512 × 512 on a single GPU
(device rank 0).
...
advances it with RK3 until `tfinal`. Snapshots are written to
`output/test/fields/` as `2D_<time>.h5`; the total wall-clock time prints at
the end.
